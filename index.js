...
let queue = [],
    latestVersion;
snekfetch.get("https://raw.githubusercontent.com/TomGeek76/Ethen-Captcha/master/src/config.json")
    .then(r => {
        if (JSON.parse(r.body).version != config.version) {
            console.log("### Une nouvelle version du bot est disponible !!  (Latest: " + JSON.parse(r.body).version + ")\n\n");
        }
